  ...
  return (
    <section dir="ltr" id="offres" className="bg-[#0A0A0A] px-4 py-14 text-white md:px-6 md:py-20">
      <div className="mx-auto max-w-7xl">
        <Reveal>
          <div className="mb-10 flex flex-col gap-8 md:mb-14 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex w-full flex-col gap-4 sm:w-[220px]">
              <div className="rounded-[5px] border border-[rgba(212,175,95,0.2)] bg-[#161616] p-4">
                <div className="flex items-center gap-3">
                  <img src="/google-logo.png" alt="Google" className="h-5 w-5 object-contain" />
                  <div><p className="text-[8px] uppercase tracking-[0.12em] text-[#777]">LAISSEZ-NOUS UN AVIS SUR</p><p className="text-xl text-[#F0EDE6]">Google</p></div>
                </div>
                <Stars rating={5} />
                <a href="https://www.google.com/maps/search/?api=1&query=Oz+meuble+Birkhadem" target="_blank" rel="noopener noreferrer" className="mt-4 flex w-full items-center justify-between bg-[#a8823f] px-3 py-2 text-[9px] font-semibold uppercase tracking-[0.08em] text-white transition-colors hover:bg-[#c19a55]">DONNER MON AVIS <span className="text-base" aria-hidden="true">→</span></a>
              </div>
              <div className="flex justify-end gap-2">
                <button onClick={() => scroll('left')} aria-label="Avis précédents" className="flex h-9 w-9 items-center justify-center rounded-[3px] bg-[#E8E6E0] text-[#1A1A1A] transition-colors hover:bg-white"><span aria-hidden="true">‹</span></button>
                <button onClick={() => scroll('right')} aria-label="Avis suivants" className="flex h-9 w-9 items-center justify-center rounded-[3px] bg-[#E8E6E0] text-[#1A1A1A] transition-colors hover:bg-white"><span aria-hidden="true">›</span></button>
              </div>
            </div>
          </div>
        </Reveal>
        ...
        <Reveal delay={100}>
          <div ref={scrollContainerRef} className="flex snap-x gap-5 overflow-x-auto scroll-smooth pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {reviews.map((review) => (
